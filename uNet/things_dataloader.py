...
class EEGDataLoader:
    def __init__(self, epochs, event_file):
        ...
        self.create_temp_dir()  # Create temp directory for image extraction
    ...
    def __getitem__(self, index):
        try:
            ...
            eeg = eeg / 10
            if eeg is None or eeg.nelement() == 0:
                return None
            image_path = Path(self.tmp_path) / 'things_stimuli' / self.events.iloc[index]['stim'].replace('\\', '/')
            if not image_path.exists() or not image_path.is_file():
                return None

            return {'eeg': eeg, 'image_path': image_path} #'image_raw': image_raw['pixel_values'],
...
def create_dataset(raw_file_path, event_description, event_file):
    
    eeg_processor = EEGDataProcessor(raw_file_path, event_description)
    eeg_processor.process_eeg_data()
    dataset = EEGDataLoader(eeg_processor.epochs, event_file)
...
if __name__ == "__main__":
    ...
    dataset, tmp_path = create_dataset(raw_file_path, eventDescription, event_file)
    ...
    # Plot the first EEG signal the 22nd channel
    plt.plot(batch['eeg'][0][22])
    plt.show()

    # Print the pixel values of the first image
    print(batch['pixel_values'][0])

...
log_name = f'log_{cur_date}_THINGS.log'
check_point_save_dir = f'checkpoint_{cur_date}_THINGS.pth'
...
def main(config):
    ...
    if finetune:
        cc = torch.load('checkpoints/pre_train.pth')
    ...
    if finetune:
        ...
        dataset_pretrain = load_and_preprocess_eeg_data(eeg_signals_path='data/eeg.pth')
        ...
        dataset_pretrain = create_dataset(raw_file_path = "data/sub-04/eeg/sub-04_task-rsvp_eeg.vhdr", event_description = 'Event/E  1', batch_size=config.batch_size)
        sampler = DistributedSampler(dataset_pretrain, rank=config.local_rank) if torch.cuda.device_count() > 1 else None 
        dataloader_eeg = DataLoader(dataset_pretrain, batch_size=config.batch_size, sampler=sampler, shuffle=sampler is None, pin_memory=True)

    logger.info(f"Start Training the autoencoder... Dataset size: {len(dataset_pretrain)}")
    ...
    for ep in range(config.num_epoch):
        ...
        if ep % 20 == 0 and config.local_rank == 0:
            save_training_checkpoint(config, ep, model_without_ddp, optimizer, loss_scaler, check_point_save_dir)
